...
def check_password():
    def password_entered():
        if st.session_state["password"] == "123456": # 这里设置你的专属密码
    ...
    if "password_correct" not in st.session_state:
        st.text_input("🛡️ 冰火防线内部数值系统 - 请输入访问口令", type="password", on_change=password_entered, key="password")
        return False
    elif not st.session_state["password_correct"]:
        st.text_input("❌ 口令错误，请重新输入", type="password", on_change=password_entered, key="password")
        return False
    else:
        return True

if not check_password():
    st.stop() # 口令不对，后面的查表逻辑全部不执行
